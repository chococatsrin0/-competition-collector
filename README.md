...
- **系统更新时间**：CSV 的「系统更新时间」优先取接口返回的 `updatedTime`（优先级：页面级 > 行级），接口无值时回退为采集时间。注意：币安官方域名（binance.com）当前不返回该字段；部分第三方网关域名在真实浏览器页面请求时会返回（可设环境变量 `BINANCE_UPDATED_TIME_HOST` 启用捕获），但实测该返回不稳定，仅建议作为参考，不作为生产依赖。
